...
// così da cambiare il suo modo di generare dati simulati
#define COMANDO_ATTUATORE
static int id;
...
sense_oxygen(){
    ...
    if(decimal_part > max_oxygen_threshold){
    	...
    	// Si accende led verde (e si spegne il rosso?)
    	leds_on(LEDS_GREEN);
    	leds_off(LEDS_RED);
    }
  ...
  while(1) {
    ...
    if((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) ||
		...
		if(state == STATE_CONNECTED){
			...
			/* Qui va messa una parte di codice di recovery
			 *  recovery()
			 */
