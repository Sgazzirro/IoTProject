...
public class DAO {
    private final String CONNECTION_URI = "jdbc:mysql://172.17.0.2:3306/IoTProject";
    ...
    public ArrayList<Measurement> readMeasure(String topic){
        ...
        String sqlStatement = "SELECT * FROM Measurements WHERE topic = ? AND STR_TO_DATE(timestamp,'%Y-%m-%d %H:%i:%s') > STR_TO_DATE(?,'%Y-%m-%d %H:%i:%s')";
        ...
        )
        {
            ...
            System.out.println("Arrivato al result set");
            while(rs.next()){
                System.out.println("Entra!");
                Measurement toAdd = new Measurement(rs.getInt(1), rs.getInt(2), rs.getDouble(3),
                        rs.getString(4), rs.getString(5));
                result.add(toAdd);
            }
    ...
    public ArrayList<CriticalSector> readCriticalMeasure(String topic, double threshold_M, double threshold_m){
        String shiftedTimestamp = readTime();
        String sqlStatement = "SELECT sector, AVG(value) FROM Measurements WHERE topic = ? AND STR_TO_DATE(timestamp,'%Y-%m-%d %H:%i:%s') > STR_TO_DATE(?,'%Y-%m-%d %H:%i:%s')  GROUP BY sector";
